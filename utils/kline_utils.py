...
def fetch_kline_data(
    ...
    Returns:
        Tuple[bool, Optional[pd.DataFrame]]:
            - 第一个元素表示是否成功
            - 第二个元素为成功时的DataFrame或失败时为None
    """
    # 加载认证配置
    if not load_auth_config():
        logger.error("无法加载认证配置")
        return False, None

    if not symbol or not exchange:
        logger.error("股票代码和交易所代码不能为空")
        return False, None
    ...
    try:
        ...
        if from_date is None:
            ...
            try:
                ...
                logger.error(f"无效的开始日期格式: {from_date}，应为YYYY-MM-DD")
                return False, None
        ...
        if to_date is None:
            ...
            try:
                ...
                logger.error(f"无效的结束日期格式: {to_date}，应为YYYY-MM-DD")
                return False, None
        ...
        if fq_date is None:
            ...
            try:
                ...
                logger.error(f"无效的复权基准日期格式: {fq_date}，应为YYYY-MM-DD")
                return False, None
        ...
        if data.get('code') == 1 and data.get('msg') == 'ok':
            ...
            if kline_data:
                ...
                logger.warning("获取K线数据成功，但数据为空")
                return False, None
        else:
            logger.error(f"获取K线数据失败: {data.get('msg', '未知错误')}")
            return False, None
