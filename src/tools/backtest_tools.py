...
async def run_strategy_backtest_with_url(
    strategy_id: str,
    listen_time: int = 30,
    start_date: Optional[str] = None,
    end_date: Optional[str] = None,
    ...
    Returns:
        str: 回测结果信息，包含CSV文件路径和HTML网址，或错误信息
    """
    try:
        # 格式化选股代码
        stock_info = None
        if choose_stock:
            stock_info = choose_stock
            # 如果不是完整的choose_stock函数代码，则格式化为选股代码
            if not choose_stock.strip().startswith("def choose_stock"):
                choose_stock = format_choose_stock(choose_stock)
        ...
        # 获取图表URL
        chart_url = get_file_url(chart_path)
        ...
        logger.error(f"运行策略回测时发生错误: {e}")
        return f"运行策略回测时发生错误: {e}"
...
def register_tools(mcp: FastMCP):
    ...
    # 注册运行策略回测并返回URL的工具
    mcp.tool()(run_strategy_backtest_with_url)
