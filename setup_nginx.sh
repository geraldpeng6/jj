...
# 配置Nginx服务器脚本
# 此脚本用于在EC2服务器上配置Nginx，以提供静态HTML文件服务
# 主要功能：
# 1. 检查并安装Nginx
# 2. 配置Nginx以提供data/charts目录中的HTML文件
# 3. 限制只能访问HTML文件，防止目录列表
# 4. 使用端口80提供服务
# 5. 重启Nginx应用配置
...
check_ec2() {
    ...
    if curl -s --connect-timeout 1 http://169.254.169.254/latest/meta-data/ > /dev/null; then
        echo -e "${GREEN}检测到在EC2实例上运行${NC}"
        return 0
    else
        echo -e "${YELLOW}警告: 未检测到在EC2实例上运行${NC}"
        echo -e "${YELLOW}此脚本主要为EC2环境设计，在其他环境中可能需要手动调整${NC}"
        return 1
    fi
}
